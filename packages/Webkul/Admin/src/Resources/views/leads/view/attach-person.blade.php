<x-admin::layouts>
    <x-slot:title>
        Persoon koppelen - {{ $lead->name }}
    </x-slot>

    <v-attach-person
        entity-name="{{ $lead->name }}"
        back-url="{{ route('admin.leads.view', $lead->id) }}"
        create-url="{{ route('admin.contacts.persons.create', ['return_to' => 'lead', 'entity_id' => $lead->id]) }}"
        search-url="{{ route('admin.contacts.persons.search') }}"
        store-url="{{ route('admin.leads.attach_person.store', $lead->id) }}"
            ...
            <div class="flex flex-col gap-4 pt-3">
                <div class="flex items-center justify-between rounded-lg border bg-white px-4 py-3 dark:border-gray-800 dark:bg-gray-900">
                    <div class="flex flex-col gap-1">
                        <span class="text-xs font-medium text-gray-500 dark:text-gray-400">Lead</span>
