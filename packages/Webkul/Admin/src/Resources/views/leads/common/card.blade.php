...
<dd class="min-w-0 flex-1">
    ...
    @if(($otherPhones && $otherPhones->count() > 0) || ($otherEmails && $otherEmails->count() > 0))
        <div class="mt-3">
            ...
            <div id="contact-{{ $lead->id }}" class="hidden mt-2 space-y-2 text-xs">
                @if($otherPhones && $otherPhones->count() > 0)
                    <div class="pl-5">
                        <div class="text-gray-500 dark:text-gray-400 mb-1">Overige telefoonnummers:</div>
                        @foreach($otherPhones as $phone)
                            <a href="tel:{{ $phone['value'] ?? '' }}" 
                               class="flex items-center gap-1.5 text-gray-700 dark:text-gray-300 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors mb-1">
                ...
                @if($otherEmails && $otherEmails->count() > 0)
                    <div class="pl-5">
                        <div class="text-gray-500 dark:text-gray-400 mb-1">Overige e-mailadressen:</div>
