@php
    use Illuminate\Support\Collection;$items = $order->orderItems ?? collect();
...
@else
    <table style="width:100%; border-collapse:collapse; margin:20px 0;">
        ...
        <tfoot>
        <tr>
            <td colspan="3" style="padding:8px; text-align:right; font-weight:600; border-top:2px solid #e5e7eb;">
                Totaal:
            </td>
            <td style="padding:8px; text-align:right; font-weight:600; border-top:2px solid #e5e7eb;">
                € {{ number_format((float)($order->total_price ?? 0), 2, ',', '.') }}</td>
        </tr>
        </tfoot>
    </table>
@endif
