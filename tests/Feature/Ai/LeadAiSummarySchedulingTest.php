...
test('refresh command can target a specific lead id including closed leads', function () {
    Queue::fake();
    ...
    $this->artisan('leads:refresh-ai-summaries', ['--lead' => [(string) $wonLead->id]])
        ->expectsOutputToContain('Queued 1 filtered lead AI summaries on [slow-lead-ai].')
        ->assertSuccessful();

    Queue::assertPushed(
        GenerateLeadAiSummaryJob::class,
        fn (GenerateLeadAiSummaryJob $job) => $job->leadId === $wonLead->id
