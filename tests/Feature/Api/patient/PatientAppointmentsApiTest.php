...
// ─── Meeting activity tests ────────────────────────────────────────────────

test('meeting activity with publish_to_portal linked via person_activities appears in appointments', function () {
    $keycloakId = 'kc-meeting-direct';
    $person = Person::factory()->create(['keycloak_user_id' => $keycloakId]);

    $activity = Activity::create([
        'type'              => ActivityType::MEETING->value,
        'title'             => 'Portal Meeting',
    ...
    $response->assertJsonStructure([
        'data' => [
            '*' => ['id', 'patient_id', 'clinic_id', 'clinic_ref', 'start_at', 'end_at', 'is_remote', 'created_at'],
        ],
    ]);
});

test('meeting activity linked via sales_lead persons appears in appointments', function () {
    $keycloakId = 'kc-meeting-saleslead';
    $person = Person::factory()->create(['keycloak_user_id' => $keycloakId]);
    ...
    $response->assertOk();
    $response->assertJsonCount(1, 'data');
    $response->assertJsonFragment(['id' => 'activity-'.$activity->id]);
});

test('meeting activity linked via lead persons appears in appointments', function () {
    $keycloakId = 'kc-meeting-lead';
    $person = Person::factory()->create(['keycloak_user_id' => $keycloakId]);
    ...
    $activity = Activity::create([
        'type'              => ActivityType::MEETING->value,
        'title'             => 'Lead Meeting',
        'schedule_from'     => now()->addDays(3)->format('Y-m-d H:i:s'),
        'schedule_to'       => now()->addDays(3)->addHour()->format('Y-m-d H:i:s'),
        'publish_to_portal' => true,
        'lead_id'           => $lead->id,
        'is_done'           => false,
    ]);
...
test('meeting activity linked via order sales_lead persons appears in appointments', function () {
    $keycloakId = 'kc-meeting-order';
    $person = Person::factory()->create(['keycloak_user_id' => $keycloakId]);
    ...
    $activity = Activity::create([
        'type'              => ActivityType::MEETING->value,
        'title'             => 'Order Meeting',
        'schedule_from'     => now()->addDays(4)->format('Y-m-d H:i:s'),
        'schedule_to'       => now()->addDays(4)->addHour()->format('Y-m-d H:i:s'),
        'publish_to_portal' => true,
        'order_id'          => $order->id,
        'is_done'           => false,
    ]);
...
test('meeting activity with publish_to_portal false is excluded from appointments', function () {
    $keycloakId = 'kc-meeting-excluded';
    $person = Person::factory()->create(['keycloak_user_id' => $keycloakId]);

    // Not published: must be excluded
    $unpublishedActivity = Activity::create([
        'type'              => ActivityType::MEETING->value,
        'title'             => 'Unpublished Meeting',
        'schedule_from'     => now()->addDay()->format('Y-m-d H:i:s'),
        'schedule_to'       => now()->addDay()->addHour()->format('Y-m-d H:i:s'),
    ...
    // Non-meeting type published: must also be excluded
    $nonMeetingActivity = Activity::create([
    ...
    DB::table('person_activities')->insert([
        ['person_id' => $person->id, 'activity_id' => $unpublishedActivity->id],
        ['person_id' => $person->id, 'activity_id' => $nonMeetingActivity->id],
    ]);
...
test('appointments endpoint merges and sorts orders and meeting activities chronologically', function () {
    $keycloakId = 'kc-meeting-merged';
    $person = Person::factory()->create(['keycloak_user_id' => $keycloakId]);

    $salesLead = SalesLead::factory()->create();
    $salesLead->persons()->attach($person->id);

    // Order scheduled for +2 hours
    $order = Order::factory()->create([
        'sales_lead_id'        => $salesLead->id,
        'pipeline_stage_id'    => PipelineStage::ORDER_WACHTEN_UITVOERING->id(),
        'first_examination_at' => now()->addHours(2),
    ]);
    $order->refresh()->update(['pipeline_stage_id' => PipelineStage::ORDER_WACHTEN_UITVOERING->id()]);

    // Activity scheduled for +1 hour (should sort before the order)
    $activity = Activity::create([
        'type'              => ActivityType::MEETING->value,
        'title'             => 'Earlier Meeting',
        'schedule_from'     => now()->addHour()->format('Y-m-d H:i:s'),
        'schedule_to'       => now()->addHours(2)->format('Y-m-d H:i:s'),
        'publish_to_portal' => true,
        'is_done'           => false,
    ]);
    ...
    expect($data[0]['id'])->toBe('activity-'.$activity->id);  // activity first (earlier)
    expect($data[1]['id'])->toBe('order-'.$order->id);         // order second (later)
});

test('future and past filters apply to meeting activities', function () {
    $keycloakId = 'kc-meeting-filter';
    $person = Person::factory()->create(['keycloak_user_id' => $keycloakId]);
    ...
    $futureResponse->assertOk();
    $futureResponse->assertJsonCount(1, 'data');
    $futureResponse->assertJsonFragment(['id' => 'activity-'.$futureMeeting->id]);
    ...
    $pastResponse->assertOk();
    $pastResponse->assertJsonCount(1, 'data');
    $pastResponse->assertJsonFragment(['id' => 'activity-'.$pastMeeting->id]);
});

test('meeting activity with clinic_id returns clinic data in response', function () {
    $keycloakId = 'kc-meeting-clinic';
    $person = Person::factory()->create(['keycloak_user_id' => $keycloakId]);
    $clinic = Clinic::factory()->create(['name' => 'Meeting Clinic']);
