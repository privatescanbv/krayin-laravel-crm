...
@php
    $aiRegistry = app(\App\Services\Ai\AiSubjectRegistry::class);
    $aiDefinition = $aiRegistry->find($subject);
    // Deliberately not gated on the enabled flag: with generation paused the last
    // summary and its corrections stay readable, only refreshing is refused.
    $aiVisible = $aiDefinition && bouncer()->hasPermission($aiDefinition->viewPermission);
@endphp

@if ($aiVisible)
    <v-ai-summary
        summary-url="{{ route('admin.ai-summary.show', [$subject, $id]) }}"
        generate-url="{{ route('admin.ai-summary.generate', [$subject, $id]) }}"
        feedback-url="{{ route('admin.ai-feedback.store', [$subject, $id]) }}"
        :can-edit="{{ bouncer()->hasPermission($aiDefinition->editPermission) ? 'true' : 'false' }}"
    ></v-ai-summary>

    @pushOnce('scripts', 'ai-summary')
    <script type="text/x-template" id="v-ai-summary-template">
    ...
    <script type="module">
        app.component('v-ai-summary', {
            template: '#v-ai-summary-template',
