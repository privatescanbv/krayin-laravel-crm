<x-admin::layouts>
    ...
    <x-admin::form :action="route('admin.clinic_departments.store')" method="POST">
        ...
        <div class="flex flex-col gap-4">
            ...
            <div class="rounded-lg border bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
                @if ($clinic_id)
                    ...
                    @php
                        $selectedClinic = $clinics->firstWhere('id', $clinic_id);
                    @endphp
                    @if ($selectedClinic)
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Kliniek</label>
                            <div class="mt-1 text-sm text-gray-900 dark:text-white">{{ $selectedClinic->name }}</div>
                        </div>
                    @endif
                @else
                    <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <textarea id="description" name="description" rows="4"
                        class="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white sm:text-sm">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea id="order_confirmation_note" name="order_confirmation_note" rows="4"
                        class="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white sm:text-sm">{{ old('order_confirmation_note') }}</textarea>
