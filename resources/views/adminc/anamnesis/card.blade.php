...
@if ($anamnesis)
    <div class="p-2 bg-activity-note-bg border border-activity-note-border rounded dark:bg-blue-900/20 dark:border-blue-800">
        <div class="flex items-center justify-between mb-1">
            <h6 class="text-xs font-semibold text-activity-note-text dark:text-activity-note-text">Anamnese</h6>
            <a
                href="{{ route('admin.anamnesis.edit', $anamnesis->id) }}"
                class="text-xs text-activity-note-text hover:text-activity-task-text dark:text-blue-400"
            ...
            @if ($conditions->isNotEmpty())
                <div class="flex flex-wrap gap-1 mt-1">
                    @foreach ($conditions as $field => $label)
                        <span class="inline-flex items-center rounded-full bg-yellow-100 text-status-on_hold-text dark:bg-yellow-900 dark:text-yellow-200 px-2 py-0.5 text-xs font-medium">
                            {{ $label }}
                        </span>
                    @endforeach
                </div>
