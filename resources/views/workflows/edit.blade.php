...
@section('page_title')
    {{ __('workflows::app.workflow.edit-title') }}
@stop

@section('content')
    <div class="content">
        <form method="POST" action="{{ route('workflows.update', $workflow->id) }}" @submit.prevent="onSubmit">
            @csrf
            @method('PUT')

            <div class="page-header">
                <div class="page-title">
                    <h1>{{ __('workflows::app.workflow.edit-title') }}</h1>
                </div>
            ...
            <div class="page-content">
                <div class="form-container">
                    <div class="control-group" :class="[errors.has('name') ? 'has-error' : '']">
                        ...
                        <input type="text" name="name" class="control" id="name" v-validate="'required'" data-vv-as="&quot;{{ __('workflows::app.workflow.name') }}&quot;" value="{{ old('name', $workflow->name) }}">
                        <span class="control-error" v-if="errors.has('name')">@{{ errors.first('name') }}</span>
                    </div>

                    <div class="control-group" :class="[errors.has('description') ? 'has-error' : '']">
                        <label for="description">{{ __('workflows::app.workflow.description') }}</label>
                        <textarea name="description" class="control" id="description" rows="5">{{ old('description', $workflow->description) }}</textarea>
                    ...
                    <div class="control-group" :class="[errors.has('pipeline_stage_id') ? 'has-error' : '']">
                        ...
                        <select name="pipeline_stage_id" class="control" id="pipeline_stage_id" v-validate="'required'" data-vv-as="&quot;{{ __('workflows::app.workflow.pipeline-stage') }}&quot;">
                            ...
                            @foreach ($pipelineStages as $pipelineStage)
                                <option value="{{ $pipelineStage->id }}" {{ old('pipeline_stage_id', $workflow->pipeline_stage_id) == $pipelineStage->id ? 'selected' : '' }}>
                                    {{ $pipelineStage->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="control-group" :class="[errors.has('lead_id') ? 'has-error' : '']">
                        ...
                        <select name="lead_id" class="control" id="lead_id">
                            ...
                            @foreach ($leads as $lead)
                                <option value="{{ $lead->id }}" {{ old('lead_id', $workflow->lead_id) == $lead->id ? 'selected' : '' }}>
                                    {{ $lead->title }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="control-group" :class="[errors.has('order_id') ? 'has-error' : '']">
                        ...
                        <select name="order_id" class="control" id="order_id">
                            ...
                            @foreach ($orders as $order)
                                <option value="{{ $order->id }}" {{ old('order_id', $workflow->order_id) == $order->id ? 'selected' : '' }}>
                                    {{ $order->id }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="control-group" :class="[errors.has('user_id') ? 'has-error' : '']">
                        ...
                        <select name="user_id" class="control" id="user_id">
                            ...
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}" {{ old('user_id', $workflow->user_id) == $user->id ? 'selected' : '' }}>
                                    {{ $user->name }}
                                </option>
                            @endforeach
                        </select>
